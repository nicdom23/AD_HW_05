 ...
 */
typedef struct {
	void*A;// the array used to store heap nodes0000
	size_t *key_pos;// the array used to store key position
	size_t *rev_pos;// the array used to store the reverse key positions
	unsigned int num_of_elem;//the number of nodes in the heap
	unsigned int num_of_elem_A;//the number of nodes in the array
	unsigned int max_size; //maximum number of nodes i.e. size of A
	size_t key_size; //size of the key type
	total_order_type leq;//the heap total order, it is memorized here
	
	//vvv maximum among the values among the binary heap
	void *max_order_value; //maximum value of the order that has been stored on the heap(don't care if its deleted or not, we just memorize it)(we do not have the "infinity" value for any randomm type and total order)
} binheap_type;
 ...
 *                 the size of A).
 * @param key_size (we pass an array of void to be general about the type of the element, then we scroll past the array in terms of bytes, this way we can find the elements of the array) 
 *                   is the type size of the values stored in A
 *
 * @param leq is the total order characterizing the heap.(provided in the dataset)
 ...
 *
 * @param H is the heap in which the key value should be decreased.
 * @param key_printer is a function to print a key value.(because the key type is yet unknown
 **********************************************************************/
